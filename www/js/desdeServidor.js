...
$(document).ready(function()  //INICIO!!!!!
  ...
  navigator.geolocation.watchPosition(actualizar, onError,{frequency:900}); //funcion actulizar
...
//127.0.0.1/bus/get   http://pruebaserver.ddns.net:443/bus/get
  $.getJSON("http://localhost:443/bus/get",function(data){ 
...
function waypoint(idBus , LatLngParadaClick)
{
  ...
  waypts = [];
  var parada=0;
//http://joseluisvelazquez.ddns.net:443/bus/get   
  $.getJSON("http://localhost:443/bus/get",function(data){
    for(index in data)
    {
      console.log(index);
          ...
          if(parada != -1) // saber si detecta bien la localizacion, si no , no se puede saber donde esta el autobus
          {
            $.getJSON("baseDatos.json",function(data){ 
              ...
              { 
                ...
                stop = new google.maps.LatLng(data[idBus][parada].lat ,data[idBus][parada].long);
                console.log(stop.lat());
                waypts.push({
                    location: stop,
                    stopover: true
